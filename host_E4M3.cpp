...
// kernel prototype
void MatMul_E4M3(data_8 A[DIM][DIM],
                 data_8 B[DIM][DIM],
                 data_8 C[DIM][DIM]);
...
int main() {
    ...
    if (!load_matrix("matrix_a_E4M3.bin", A))    return 1;
    if (!load_matrix("matrix_b_E4M3.bin", B))    return 1;
    if (!load_matrix("matrix_c_E4M3.bin", C_ref))return 1;

    MatMul_E4M3(A,B,C_hw);

    // compute MSE
    double mse=0.0;
    for(int i=0;i<DIM;i++) for(int j=0;j<DIM;j++){
        double df = double(e4m3_to_float(C_hw[i][j]))
                  - double(e4m3_to_float(C_ref[i][j]));
        mse += df*df;
    }
    mse /= double(DIM)*double(DIM);
    std::cout << "E4M3 MSE = " << mse << "\n";
    return 0;
}

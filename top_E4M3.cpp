...
//-----------------------------------------------------------------------------
// Convert 8-bit E4M3 → float32
//-----------------------------------------------------------------------------
static float e4m3_to_float(data_8 hbits) {
    ...
    if (exp == 0) {
        if (frac == 0) {
            ...
            mant &= ((1u << m) - 1);
            int32_t new_exp = 127 - bias - shift + 1; // = 121 - shift
...
//-----------------------------------------------------------------------------
// Convert float32 → 8-bit E4M3 (round-to-nearest)
//-----------------------------------------------------------------------------
static data_8 float_to_e4m3(float vf) {
    ...
    const int bias          = 7;
    const int m             = 3;
    const int maxExpField   = 0xF;       // 4 bits all 1
    const int Emax_normal   = (maxExpField - 1) - bias; // 14 - 7 = 7
    const int Emin_normal   = 1 - bias;  // -6
...
#pragma HLS INTERFACE s_axilite port=return bundle=control
    ...
    for (int i = 0; i < DIM; i++) {
        for (int j = 0; j < DIM; j++) {
            float sum = 0.0f;
            for (int k = 0; k < DIM; k++) {
                float fa = e4m3_to_float(A[i][k]);
                float fb = e4m3_to_float(B[k][j]);
                sum += fa * fb;
            }
            C[i][j] = float_to_e4m3(sum);
        }
    }
}
